...
[Copy page](https://openai.com/index/computer-using-agent/)
...
![Computer use diagram](https://cdn.openai.com/API/docs/images/cua_diagram.png ':size=1000')
    ...
    try:
        match action_type:
            ...
            case "click":
                ...
                docker_exec(
                    f"DISPLAY={vm.display} xdotool mousemove {x} {y} click {b}",
                    vm.container_name,
                )
            ...
            case "scroll":
                ...
                docker_exec(
                    f"DISPLAY={vm.display} xdotool mousemove {x} {y}", vm.container_name
                )
            ...
            case "keypress":
                ...
                for k in keys:
                    ...
                    if k.lower() == "enter":
                        docker_exec(
                            f"DISPLAY={vm.display} xdotool key 'Return'",
                            vm.container_name,
                        )
                    elif k.lower() == "space":
                        docker_exec(
                            f"DISPLAY={vm.display} xdotool key 'space'",
                            vm.container_name,
                        )
                    else:
                        docker_exec(
                            f"DISPLAY={vm.display} xdotool key '{k}'", vm.container_name
                        )
            ...
            case "type":
                ...
                print(f"Action: type text: {text}")
                docker_exec(
                    f"DISPLAY={vm.display} xdotool type '{text}'", vm.container_name
                )
...
def get_screenshot(vm):
    ...
    """
    cmd = f"export DISPLAY={vm.display} && " "import -window root png:-"
