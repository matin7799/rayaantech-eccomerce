...
|------|----------------|-------------|
| Basic (Tier 0) | Sign up with an email address | See [Basic Tier Limits](en/rate-limits-tier0.md) |
...
The [Files API](en/api-reference/files.md) (`/v1/files`) has separate rate limits for file operations. These limits are tier-based and apply per minute.

> **🎉 Free Beta Program**: All v1/files operations are completely **FREE** from **January 1, 2026** through **March 1, 2026** (60 days). We encourage you to test and report any issues to [t.me/AvalAISupport](https://t.me/AvalAISupport).

### File Operations Rate Limits (per minute)
...
Each tier has a total storage limit. Once exceeded, uploads are blocked until you free space by deleting files or upgrade to a higher tier.
...
**File Size Limit**: Maximum upload size is **128 MB** per file (during beta).

For complete Files API documentation including endpoints, code examples, and supported file purposes, see the [Files API Reference](en/api-reference/files.md).
